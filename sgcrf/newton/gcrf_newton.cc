...
// TODO(mwytock): Consider moving more things in here (Lambda, Theta, Sigma,
// etc.)  
// TODO(mwytock): Consider a design that lazily evaluates these products and
// caches the result?
...
int NumberNonzeros(const MatrixXd& Lambda, const MatrixXd& Theta) {
  int nnz_lambda = Lambda.count();
  int nnz_lambda_diag = Lambda.diagonal().count();
  int nnz_theta = Theta.count();
  return nnz_theta + nnz_lambda_diag + (nnz_lambda - nnz_lambda_diag)/2;
}
...
// TODO(mwytock): Objective should take IterationState as input. 
double Objective(
  const Covariance& S, 
  double lambda, 
  const MatrixXd& Lambda, 
...
void CoordinateDescent(
  ...
  // TODO(mwytock): Update these names to be consistent with IterationState
  ...
  for (int t = 0; t < max_iters; t++) {
    ...
    double normV = V.norm()/sqrt(n*p);
    double diffU = (U - Uold).norm()/p;
    double diffV = (V - Vold).norm()/sqrt(n*p);
...
void OptimizeGCRF(
  ...
  for (int t = 0; t < params.max_iters; t++) {
    ...
    int n_active_set = active_lambda.size() + active_theta.size() + p;
    int nnz = NumberNonzeros(Lambda, Theta); 

    if (subgrad < params.tol || subgrad < params.tol*lambda*l1_norm) {
      if (!params.quiet) {
        PRINTF("Converged, subgrad: %f, l1 norm: %f, nnz: %d\n", 
               subgrad, l1_norm, nnz);
      }
      break;
    }
    ...
    if (!params.quiet) {
      PRINTF("Newton iteration %d, subgrad: %f, l1 norm: %f, nnz: %d, active set size: %d\n", 
             t, subgrad, l1_norm, nnz, n_active_set);
    }
    ...
    CoordinateDescent(
      ...
      params.cd_max_iters == 0 ? 1 + t/3 : params.cd_max_iters, state, U, V);
